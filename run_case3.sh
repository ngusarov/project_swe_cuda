...
TEST_CASE_ID=3 # Tsunami from file
TEND=0.2       # Simulation time in hours for this case
REPEAT=${1:-3} # Number of repetitions, default to 3

# Problem sizes (these correspond to the 'nx' in the HDF5 filename, e.g., Data_nx101_500km.h5)
# The actual nx/ny passed to the solver will be SIZE, but the HDF5 file will be based on SIZE+1
...
echo "Starting benchmarks for Test Case ${TEST_CASE_ID} (Tsunami from File)"
...
for SIZE in "${SIZES[@]}"; do
  ...
  for TPB in "${TPBS[@]}"; do
    for R in $(seq 1 "$REPEAT"); do
      ...
      # Execute the SWE solver with arguments: test_case_id nx ny Tend threadsPerBlock [output_fname]
      # For test_case_id 3, nx and ny are used to determine the HDF5 filename (nx+1)
